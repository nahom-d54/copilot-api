...
import { parseModelName } from "~/lib/model-routing"
...
export async function handleResponses(c: Context) {
  ...
  // Parse model suffix for reasoning effort
  const { model, reasoningEffort } = parseModelName(payload.model)
  const finalPayload: ResponsesPayload = {
    ...payload,
    model,
  ...
  const response = await createResponses(finalPayload)
